"""
Laptop Monitor real-hardware agent -- runs on a physical Windows laptop
and feeds real telemetry into the Laptop Monitor backend
(../backend, see app/orchestrator.py), which decides whether to prompt
this machine to sleep or ask an operator to approve raising its fan
speed.
...
What it does, every poll cycle:
  ...
  2. POSTs that sample to the backend's /api/telemetry endpoint.
  3. Polls /api/commands/{host_id} for anything the backend wants this
     machine to do, and acts on it:
       - "sleep_prompt": shows a real Yes/No prompt on this machine
         ("this laptop has been idle for N minutes -- sleep now?"). Only
         a Yes actually suspends the machine.
  ...
  4. Acks back what actually happened via /api/commands/{id}/ack, so the
     dashboard reflects reality, not just what was requested.

Only depends on the standard library + psutil + requests.
"""
...
logger = logging.getLogger("laptop-monitor.agent")
...
def get_idle_seconds() -> float:
    ...
    Win32 GetLastInputInfo idle-detection pattern. ctypes-only."""
    info = LASTINPUTINFO()
...
def get_cpu_temp_c() -> Optional[float]:
    ...
    (namespace root/LibreHardwareMonitor, class Sensor) -- see README.md
    for the one-time setup. Returns None (not a fabricated value) if
    that provider isn't reachable.
    """
    try:
        import wmi  # optional dependency, only needed for this
...
class ThroughputMeter:
    ...
    against a configured max throughput."""
...
# --------------------------------------------------------------------------
# Real actuation -- always asks first for sleep, never hard-codes a
...
def handle_sleep_prompt(config: Config, command: dict, session: requests.Session) -> None:
    ...
    message = f"This laptop has been idle for ~{idle_minutes} minutes.\n\nPut it to sleep now?"
    answer = ctypes.windll.user32.MessageBoxW(None, message, "Laptop Monitor", MB_YESNO_ICONQUESTION)
    if answer != IDYES:
...
def ack(session: requests.Session, config: Config, command_id: str, result: str, detail: str = "") -> None:
    try:
        session.post(
            f"{config.backend_url}/api/commands/{command_id}/ack",
...
def run(config: Config) -> None:
    ...
    while True:
        try:
            ...
            session.post(f"{config.backend_url}/api/telemetry", json=sample, timeout=10)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Laptop Monitor real-hardware telemetry agent")
